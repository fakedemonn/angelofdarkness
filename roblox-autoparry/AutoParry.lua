...
do
		...
		return function(ctx)
			...
			function Dodge.dash(requested)
				...
				if mode == "Double Tap" then
					...
					local dashKey = Options and Options.DashKey and Options.DashKey.Value or "Q"
			...
			---Parry was refused; should we dash instead, and if so, do it.
			---
			---Only cooldown and a manually held key are worth dashing through. "disabled"
			---and "dead" are not: dashing when the whole feature is off, or when you are
			---a corpse, is noise.
			---@param reason string the refusal from State.canParry
			...
			function Dodge.fallback(reason, timing)
				...
				if reason ~= "cooldown" and reason ~= "key held manually" then
					return false
				end

				local requested = timing and timing.dodgeDir
				if requested == "None" or requested == "Back" then
					-- The timing's own dodgeDir uses the parry-time vocabulary, which is a
					-- different set of words from the dash vocabulary. Map the one that
					-- actually differs and let the rest fall through to Auto.
					requested = requested == "Back" and "Backward" or nil
				end

				local fired = Dodge.dash(requested)
				return fired
			end
	...
	local ok, err = pcall(factory, ctx)
	if not ok then
		error("[AutoParry] error in src/features/Dodge.lua: " .. tostring(err), 0)
	end
end
	...
	local factory = (function()
		...
		return function(ctx)
			...
			function Engine.onAnimation(entity, track)
				...
				-- Draw this attack's own gate on the attacker for the length of the swing.
				...
				if timing and ctx.Hitbox and ctx.Hitbox.flash then
					ctx.Hitbox.flash(entity, timing, math.max(track.Length / math.max(track.Speed, 0.01), 0.4))
				end
...
do
	...
	local ok, err = pcall(factory, ctx)
	if not ok then
		error("[AutoParry] error in src/features/Effects.lua: " .. tostring(err), 0)
	end
end
...
do
	local factory = (function()
		...
		return function(ctx)
			...
			DodgeBox:AddToggle("AutoDodgeOnCooldown", {
				Text = "Dash If Parry Busy",
				Default = false,
				Tooltip = "When an attack fires while parry is on cooldown, dash out of it instead",
			})
...
do
	...
	local ok, err = pcall(factory, ctx)
	if not ok then
		error("[AutoParry] error in src/ui/EffectsTab.lua: " .. tostring(err), 0)
	end
end
...
do
	local factory = (function()
		...
		return function(ctx)
			...
			outer.Size = UDim2.new(0, 570, 0, 360)
			outer.Parent = screen
			...
			saveButton.Position = UDim2.new(0, PANEL_X, 0, 310)
			...
			toggleButton.Position = UDim2.new(0, PANEL_X, 0, 334)
...
do
	...
	local ok, err = pcall(factory, ctx)
	if not ok then
		error("[AutoParry] error in src/ui/EffectWindow.lua: " .. tostring(err), 0)
	end
end
	...
	local factory = (function()
		...
		return function(ctx)
			...
			StoreBox:AddButton({
				...
				Func = function()
					-- The export path when Write To Disk is off. Your tuning still has
					-- somewhere to go; it just does not land in a file by itself.
	...
	local factory = (function()
		...
		return function(ctx)
			...
			Library:OnUnload(function()
				...
				if Store.dirty then
					-- With Write To Disk off this cannot persist anything, so say so out
					-- loud rather than silently dropping the session's tuning. Copy
					-- Database on the Builder tab is the way out.
					-- warn, not notify: the notification GUI is about to be destroyed
					-- along with the rest of the library, so it would flash and vanish.
					local ok = Store.save(Store.configName)
					if not ok then
						warn("[AutoParry] Unsaved timing changes - Write To Disk was off. Use Copy Database next time.")
					end
				end
